...
def test_switch_season_invalid_season_value(client, mock_dashboard_service):
    ...
    assert response.status_code == 400
    assert response.json() == {
        "detail": "Invalid season value: 6",
    }


# def test_switch_season_connect_to_edge_server_error(
#     client, mock_dashboard_service, mock_edge_server
# ):
#     mock_dashboard_service.switch_season_mode.side_effect = ConnectToEdgeServerError(
#         "Connect to Edge server error."
#     )
#     response = client.post("/api/switch-season", json={"season_value": 0})
