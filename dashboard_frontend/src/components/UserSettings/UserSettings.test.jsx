...
describe('UserSettings Component', () => {
  ...
  test('displays error message when updateSettings fails with read-only mode error', async () => {
    ...
    updateSettings.mockRejectedValue({ data: { detail: 'Operation not permitted: system is in read-only mode' } });
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith('Operation not permitted: system is in read-only mode');
    });
  });

  test('displays success message when updateSettings succeeds', async () => {
    // Simulate a successful API call
    updateSettings.mockResolvedValue({ data: { message: 'Settings updated successfully' } });
