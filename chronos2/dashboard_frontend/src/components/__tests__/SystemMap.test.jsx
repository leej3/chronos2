...
describe("SystemMap component", () => {
  ...
  it("should render system map for Winter season with boiler status", () => {
    ...
    expect(screen.getByText((content) => content.includes("50") && content.includes("°F"))).toBeInTheDocument();
    expect(screen.getByText((content) => content.includes("40") && content.includes("°F"))).toBeInTheDocument();
  });
  ...
  it("should render system map for Summer season with chiller status", () => {
    ...
    expect(screen.getByText((content) => content.includes("70") && content.includes("°F"))).toBeInTheDocument();
    expect(screen.getByText((content) => content.includes("60") && content.includes("°F"))).toBeInTheDocument();
  });
  ...
  it("should show loading screen when season is not set", () => {
    ...
    const loadingContainer = container.querySelector('.spinner-border');
