...
def mock_manager():
    with patch(
        "chronos.devices.manager.get_device_manager", return_value=MockDeviceManager()
    ) as mock:
        yield mock
...
def test_get_data(client, mock_manager):
    ...
    assert data["mock_devices"] is True  # Kiểm tra giá trị đúng


# Vì đang ép mock mode
