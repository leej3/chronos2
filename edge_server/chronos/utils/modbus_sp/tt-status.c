...
int main(int argc, char **argv)
{
	...
	int16_t regs[9];	/* Holds results of register reads */
...
/* #warning slave ID needs to be an option too */
	...
	/* System Supply Temp */
  printf("System Supply Temp:    %5.1f °C\n", 1.0*regs[6]/10);
	...
	/* System Supply Setp */
  printf("System Supply Setp:    %5.1f °C\n", 1.0*regs[0]/2);
	/* Cascade Current Power */
  printf("Cascade Current Power: %5.1f %\n", 1.0*regs[3]);
	/* Outlet Setp */
  printf("Outlet Setp:           %5.1f °C\n", 1.0*regs[4]/10);
	/* Outlet Temp */
  printf("Outlet Temp:           %5.1f °C\n", 1.0*regs[5]/10);
	/* Intlet Temp */
  printf("Inlet Temp:            %5.1f °C\n", 1.0*regs[6]/10);
	/* Flue Temp */
  printf("Flue Temp:             %5.1f °C\n", 1.0*regs[7]/10);
	/* Firing Rate */
